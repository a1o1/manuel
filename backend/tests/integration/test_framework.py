...
class MockAWSServices:
    ...
    def _create_mock_resources(self):
        ...
        self.error_table = self.dynamodb.create_table(
            ...
            KeySchema=[{"AttributeName": "error_id", "KeyType": "HASH"}],
            AttributeDefinitions=[{"AttributeName": "error_id", "AttributeType": "S"}],
            BillingMode="PAY_PER_REQUEST",
        )

        self.rate_limit_table = self.dynamodb.create_table(
            TableName="manuel-rate-limits-test",
            KeySchema=[{"AttributeName": "limit_key", "KeyType": "HASH"}],
            AttributeDefinitions=[{"AttributeName": "limit_key", "AttributeType": "S"}],
            BillingMode="PAY_PER_REQUEST",
        )
...
class IntegrationTestFramework:
    ...
    def _run_single_test(self, test_name: str, test_func: Callable):
        ...
        try:
            ...
            result = TestResult(
                test_name=test_name, status="passed", duration_ms=duration_ms
            )
        ...
        report = {
            "summary": {
                ...
                "average_test_duration_ms": (
                    total_duration / self.total_tests if self.total_tests > 0 else 0
                ),
            },
    ...
    def _test_transcribe_endpoint(self):
        ...
        response = self._make_api_request(
            "POST", "/api/transcribe", headers=headers, json=data
        )
    ...
    def _test_query_endpoint(self):
        """Test query endpoint"""
        headers = self.authenticator.get_auth_headers()
        data = {"question": self.test_data["test_question"]}

        response = self._make_api_request(
            "POST", "/api/query", headers=headers, json=data
        )
        assert response.status_code == 200
    ...
    def _test_sql_injection_protection(self):
        ...
        headers = self.authenticator.get_auth_headers()
        malicious_data = {"question": "'; DROP TABLE users; --"}

        response = self._make_api_request(
            "POST", "/api/query", headers=headers, json=malicious_data
        )
    ...
    def _test_xss_protection(self):
        """Test XSS protection"""
        headers = self.authenticator.get_auth_headers()
        malicious_data = {"question": '<script>alert("XSS")</script>'}
    ...
    def _test_request_size_limits(self):
        ...
        large_data = {"question": "A" * 1000000}  # 1MB question

        response = self._make_api_request(
            "POST", "/api/query", headers=headers, json=large_data
        )
    ...
    def _test_cache_performance(self):
        ...
        start_time = time.time()
        response1 = self._make_api_request(
            "POST", "/api/query", headers=headers, json=data
        )
        ...
        start_time = time.time()
        response2 = self._make_api_request(
            "POST", "/api/query", headers=headers, json=data
        )
    ...
    def _test_complete_voice_query_flow(self):
        ...
        response = self._make_api_request(
            "POST", "/api/transcribe", headers=headers, json=transcribe_data
        )
        assert response.status_code == 200
        ...
        query_data = {"question": transcribe_result["transcription"]}

        response = self._make_api_request(
            "POST", "/api/query", headers=headers, json=query_data
        )
        assert response.status_code == 200
    ...
    def _test_quota_enforcement_flow(self):
        ...
        assert (
            updated_usage["daily_usage"]["used"] > initial_usage["daily_usage"]["used"]
        )
