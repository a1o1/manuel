...
class ManuelCostCalculator:
    ...
    def __init__(self, region: str = "eu-west-1"):
        ...
        self.pricing = {
            ...
            "transcribe": {
                "standard": 0.0004  # $0.0004 per second
            },
            "lambda": {
            ...
            "api_gateway": {
                "requests": 0.0000035  # $3.50 per million requests
            },
            "cloudwatch": {
    ...
    def calculate_bedrock_cost(self, model_id: str, input_tokens: int, 
                              output_tokens: int = 0) -> ServiceCost:
        """Calculate cost for Bedrock model inference"""
        if model_id not in self.pricing["bedrock"]:
            # Default to Claude 3.5 Sonnet pricing
            model_pricing = self.pricing["bedrock"]["anthropic.claude-3-5-sonnet-20241022-v2:0"]
        else:
    ...
    def calculate_lambda_cost(self, duration_ms: float, memory_mb: int) -> ServiceCost:
        ...
        compute_cost = (memory_gb * duration_seconds) * self.pricing["lambda"]["compute_gb_second"]
    ...
    def calculate_dynamodb_cost(self, read_units: int = 0, write_units: int = 0) -> ServiceCost:
        ...
        return ServiceCost(
            ...
            unit="request_units",
            unit_cost=total_cost / (read_units + write_units) if (read_units + write_units) > 0 else 0,
            total_cost=total_cost
        )
    
    def calculate_s3_cost(self, storage_gb: float = 0, put_requests: int = 0, 
                         get_requests: int = 0) -> ServiceCost:
        """Calculate cost for S3 operations"""
        # Monthly storage cost (prorated)
        storage_cost = storage_gb * self.pricing["s3"]["standard_storage"] / 30 / 24  # Per hour
    ...
    def calculate_request_cost(self, request_id: str, user_id: str, operation: str,
                              **service_params) -> RequestCost:
        """
        ...
        if "lambda_duration_ms" in service_params and "lambda_memory_mb" in service_params:
            service_costs.append(self.calculate_lambda_cost(
        ...
        if "dynamodb_reads" in service_params or "dynamodb_writes" in service_params:
            service_costs.append(self.calculate_dynamodb_cost(
                service_params.get("dynamodb_reads", 0),
                service_params.get("dynamodb_writes", 1)  # At least one write for quota
            ))
        
        # Operation-specific costs
        if operation == "transcribe":
            if "transcribe_duration_seconds" in service_params:
                service_costs.append(self.calculate_transcribe_cost(
                    service_params["transcribe_duration_seconds"]
                ))
            
            if "s3_put_requests" in service_params:
                service_costs.append(self.calculate_s3_cost(
            ...
            if "embedding_tokens" in service_params:
                embedding_model = service_params.get("embedding_model", "amazon.titan-embed-text-v2:0")
                service_costs.append(self.calculate_bedrock_cost(
                    embedding_model,
                    service_params["embedding_tokens"]
                ))
            
            # Bedrock text generation cost
            if "text_input_tokens" in service_params and "text_output_tokens" in service_params:
                text_model = service_params.get("text_model", "anthropic.claude-3-5-sonnet-20241022-v2:0")
                service_costs.append(self.calculate_bedrock_cost(
                    text_model,
        ...
        try:
            ...
            table.put_item(
                Item={
                    ...
                    'total_cost': str(request_cost.total_cost),  # Store as string to avoid precision issues
                    'service_costs': json.dumps([asdict(cost) for cost in request_cost.service_costs]),
