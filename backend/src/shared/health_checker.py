...
class CircuitBreaker:
    ...
    def __init__(self, service_name: str, failure_threshold: int = 5, 
                 timeout_seconds: int = 60, success_threshold: int = 2):
...
class ManuelHealthChecker:
    ...
    def __init__(self):
        self.circuit_breakers = {
            "bedrock": CircuitBreaker("bedrock", failure_threshold=3, timeout_seconds=30),
            "transcribe": CircuitBreaker("transcribe", failure_threshold=3, timeout_seconds=30),
            "dynamodb": CircuitBreaker("dynamodb", failure_threshold=5, timeout_seconds=60),
            "s3": CircuitBreaker("s3", failure_threshold=5, timeout_seconds=60),
            "knowledge_base": CircuitBreaker("knowledge_base", failure_threshold=3, timeout_seconds=45)
        }
    
    def perform_health_check(self, include_deep_checks: bool = True) -> SystemHealthReport:
    ...
    def _check_bedrock(self) -> HealthCheckResult:
        ...
        try:
            def check_bedrock():
                ...
                model_id = os.environ.get('TEXT_MODEL_ID', 'anthropic.claude-3-5-sonnet-20241022-v2:0')
