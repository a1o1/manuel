...
class SecurityMiddleware:
    ...
    def __init__(self, function_name: str):
        ...
        # Security thresholds
        self.max_request_size = int(os.environ.get('MAX_REQUEST_SIZE_MB', '10')) * 1024 * 1024
        self.max_audio_duration = int(os.environ.get('MAX_AUDIO_DURATION_SECONDS', '300'))
    ...
    def validate_request(self, event: Dict[str, Any], context: Any) -> Tuple[bool, Optional[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            if not self._validate_request_size(event):
                return False, create_response(413, {
            ...
            if not self._check_rate_limit(event):
                return False, create_response(429, {
            ...
            return False, create_response(500, {
        ...
        try:
            ...
            if body:
                ...
                if size > self.max_request_size:
                    self.logger.warning("Request size exceeded", 
                                      size_bytes=size,
    ...
    def _validate_ip_address(self, event: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.warning("IP address not in allowlist", 
                              source_ip=source_ip,
    ...
    def _check_content_patterns(self, content: str) -> bool:
        ...
        for pattern in self.xss_patterns:
            if re.search(pattern, content, re.IGNORECASE):
                self.logger.warning("XSS pattern detected", 
                                  pattern=pattern,
                                  content_preview=content[:100])
                return False
    ...
    def _validate_request_signature(self, event: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            path = event.get('path', '')
            if not any(sensitive in path for sensitive in ['/admin/', '/upload', '/delete']):
                return True
            
            # Check for signature header
            signature_header = event.get('headers', {}).get('X-Manuel-Signature', '')
            if not signature_header:
                self.logger.warning("Missing signature for sensitive operation", path=path)
                return False
