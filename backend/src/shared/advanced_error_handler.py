...
class AdvancedErrorHandler:
    ...
    def __init__(self, function_name: str, context: Any = None):
        ...
        self.sqs_client = boto3.client(
            "sqs", config=Config(retries={"max_attempts": 3}, max_pool_connections=10)
        )
        self.sns_client = boto3.client(
            "sns", config=Config(retries={"max_attempts": 3})
        )
        ...
        try:
            ...
            if severity in [ErrorSeverity.HIGH, ErrorSeverity.CRITICAL]:
                self._send_error_notification(
                    exception, service, error_context, severity
                )
    ...
    def _send_to_dlq(
        self, exception: Exception, service: str, error_context: ErrorContext
    ):
        ...
        try:
            ...
            if hasattr(exception, "response"):
                ...
                message["exception"]["response_metadata"] = exception.response.get(
                    "ResponseMetadata", {}
                )
            ...
            self.sqs_client.send_message(
                ...
                MessageAttributes={
                    "Service": {"StringValue": service, "DataType": "String"},
                    "Severity": {
            ...
            self.logger.error(
                "Failed to send message to DLQ", error=str(e), dlq_url=self.dlq_url
            )
    ...
    def _track_error(
        ...
        try:
            ...
            # Add AWS-specific error details
            if hasattr(exception, "response"):
                item["aws_error_code"] = exception.response.get("Error", {}).get(
                    "Code", ""
                )
                item["aws_error_message"] = exception.response.get("Error", {}).get(
                    "Message", ""
                )
                item["http_status_code"] = exception.response.get(
                    "ResponseMetadata", {}
...
"""
            ...
            self.sns_client.publish(
                ...
                Message=message,
                MessageAttributes={
                    "Severity": {"StringValue": severity.value, "DataType": "String"},
                    "Service": {"StringValue": service, "DataType": "String"},
                    "FunctionName": {
                        "StringValue": error_context.function_name,
                        "DataType": "String",
...
def error_handler_decorator(
    service: str, default_message: str = "An unexpected error occurred"
):
        ...
        def wrapper(event, context):
            ...
            try:
                ...
                error_context = ErrorContext(
                    ...
                    error_details={"event": event, "function_name": func.__name__},
                )
