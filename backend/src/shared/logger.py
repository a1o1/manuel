...
class ManuelLogger:
    ...
    def metric(self, metric_name: str, value: float, unit: str = 'Count', **dimensions) -> None:
        ...
        try:
            ...
            # Also log the metric
            self.info("Custom metric emitted", 
            ...
            self.error("Failed to emit custom metric", 
                      metric_name=metric_name, 
                      error=str(e))
    ...
    def log_request_end(self, status_code: int, duration_ms: float, **kwargs) -> None:
        ...
        # Emit custom metrics
        self.metric("RequestDuration", duration_ms, "Milliseconds", 
                   Function=self.function_name)
        self.metric("RequestCount", 1, "Count", 
                   Function=self.function_name, 
                   StatusCode=str(status_code))
    
    def log_quota_check(self, user_id: str, operation: str, can_proceed: bool, usage_info: Dict[str, Any]) -> None:
        """Log quota check results"""
        self.info("Quota check performed",
                 user_id=user_id,
                 operation=operation,
        ...
        if 'daily_used' in usage_info and 'daily_limit' in usage_info:
            quota_percentage = (usage_info['daily_used'] / usage_info['daily_limit']) * 100
            self.metric("QuotaUsagePercentage", quota_percentage, "Percent",
                       QuotaType="Daily", Operation=operation)
        
        if not can_proceed:
            self.metric("QuotaExceeded", 1, "Count", 
                       Operation=operation, QuotaType="Daily")
    
    def log_bedrock_call(self, model_id: str, operation: str, duration_ms: float, 
                        tokens_used: Optional[int] = None, success: bool = True) -> None:
        ...
        # Emit metrics
        self.metric("BedrockCallDuration", duration_ms, "Milliseconds",
                   Model=model_id, Operation=operation)
        self.metric("BedrockCallCount", 1, "Count",
                   Model=model_id, Operation=operation, 
                   Status="Success" if success else "Failure")
        
        if tokens_used:
            self.metric("BedrockTokens", tokens_used, "Count",
                       Model=model_id, Operation=operation)
    
    def log_transcription(self, audio_size_bytes: int, transcription_length: int, 
    ...
    def log_knowledge_base_query(self, query: str, results_count: int, 
        ...
        self.metric("KnowledgeBaseQueryCount", 1, "Count",
                   Status="Success" if success else "Failure")
