        ...
        for record in event.get("Records", []):
            try:
                ...
                error_data = {
                    ...
                    "exception_type": message_body.get("exception", {}).get(
                        "type", "Unknown"
                    ),
                    "exception_message": message_body.get("exception", {}).get(
                ...
                response_metadata = message_body.get("exception", {}).get(
                    "response_metadata", {}
                )
                if response_metadata:
                    error_data["http_status_code"] = response_metadata.get(
                        "HTTPStatusCode", 0
                    )
                ...
                if error_data["severity"] in ["high", "critical"] and error_topic_arn:
                    _send_error_notification(
                        sns_client, error_topic_arn, error_data, logger
                    )
                ...
                failed_count += 1
                logger.error(
                    "Failed to process error record",
                    error=str(e),
...
def _determine_severity(message_body: Dict[str, Any]) -> str:
    ...
    if (
        error_code
        in ["LimitExceededException", "QuotaExceededException", "RequestLimitExceeded"]
        or http_status >= 500
...
def _send_error_notification(
    sns_client, topic_arn: str, error_data: Dict[str, Any], logger
):
    """Send error notification via SNS"""
    try:
        subject = (
            f"Manuel {error_data['severity'].upper()} Error - {error_data['service']}"
        )
        message = f"""
...
"""
        ...
        sns_client.publish(
            ...
            MessageAttributes={
                ...
                "Service": {"StringValue": error_data["service"], "DataType": "String"},
                "FunctionName": {
