...
class S3Optimizer:
    ...
    def upload_file_optimized(
        ...
        try:
            ...
            self.logger.error(f"S3 upload failed: {str(e)}", bucket=bucket, key=key, size=file_size)
    ...
    def _upload_part(self, bucket: str, key: str, upload_id: str, part_number: int, data: bytes) -> str:
    ...
    def get_object_metadata_cached(self, bucket: str, key: str) -> Optional[Dict[str, Any]]:
    ...
    def download_object_optimized(self, bucket: str, key: str) -> Tuple[bytes, Dict[str, Any]]:
    ...
    def _download_large_object(self, bucket: str, key: str, size: int) -> Tuple[bytes, Dict[str, Any]]:
    ...
    def batch_delete_objects(self, bucket: str, keys: List[str]) -> Dict[str, Any]:
        ...
        for i in range(0, len(keys), batch_size):
            ...
            try:
                ...
                self.logger.error(f"Batch delete failed: {str(e)}", bucket=bucket, batch_size=len(batch_keys))
                errors.extend([{"Key": key, "Code": "DeleteError", "Message": str(e)} for key in batch_keys])
