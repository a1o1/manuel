...
from utils import calculate_ttl, get_current_date, get_current_month
...
class CachedQuotaManager:
    ...
    def _init_redis_cache(self):
        ...
        try:
            if (
                os.environ.get("ENABLE_PERFORMANCE_OPTIMIZATION", "false").lower()
                == "true"
            ):
    ...
    def _set_memory_cache(self, cache_key: str, data: Dict[str, Any]):
        ...
        self.memory_cache[cache_key] = {"data": data, "timestamp": time.time()}
    ...
    def _set_redis_cache(self, cache_key: str, data: Dict[str, Any]):
        ...
        try:
            self.redis_client.setex(cache_key, self.cache_ttl_seconds, json.dumps(data))
        except Exception as e:
    ...
    def increment_usage_atomic(
        ...
        try:
            ...
            response = self.table.update_item(
                ...
                """,
                ExpressionAttributeNames={"#month": "month", "#ttl": "ttl"},
