...
def handle_backup_status(logger) -> Dict[str, Any]:
    ...
    try:
        ...
        return create_response(
            500, {"error": "Failed to get backup status", "details": str(e)}
        )
...
def handle_backup_verify(event: Dict[str, Any], logger) -> Dict[str, Any]:
    ...
    try:
        ...
        status_code = (
            200 if verification_results["overall_status"] == "healthy" else 207
        )
        ...
        return create_response(
            500, {"error": "Backup verification failed", "details": str(e)}
        )
...
def handle_disaster_recovery(event: Dict[str, Any], logger) -> Dict[str, Any]:
    ...
    try:
        ...
        return create_response(
            500, {"error": "Disaster recovery failed", "details": str(e)}
        )
...
def handle_backup_metrics(logger) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info("Backup metrics retrieved", metrics_count=len(metrics))
        ...
        return create_response(
            500, {"error": "Failed to get backup metrics", "details": str(e)}
        )
...
def check_dynamodb_backup_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if not table_name:
            return {"status": "unknown", "error": "Table name not configured"}
        ...
        return {
            "status": (
                "healthy"
                if pitr_status["PointInTimeRecoveryStatus"] == "ENABLED"
                else "unhealthy"
            ),
...
def check_s3_backup_status() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            status_info["primary_bucket"]["versioning"] = versioning.get(
                "Status", "Suspended"
            )
        except ClientError:
...
def verify_dynamodb_backup(logger) -> Dict[str, Any]:
    ...
    try:
        ...
        if not table_name:
            return {"status": "failed", "error": "Table name not configured"}
