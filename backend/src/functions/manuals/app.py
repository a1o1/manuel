...
from url_downloader import get_secure_url_downloader, DownloadConfig
from utils import (
    create_response,
...
def handle_download_manual(event: Dict[str, Any], user_id: str, logger, context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if not can_proceed:
            logger.warning("Quota exceeded for URL download", user_id=user_id, quota_info=quota_info)
        ...
        if not download_result.success:
            logger.warning("URL download failed", url=url, error=download_result.error_message)
        ...
        logger.error(f"Error downloading manual from URL: {str(e)}", url=url, user_id=user_id)
