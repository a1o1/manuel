...
// import { authService as sharedAuthService } from '@manuel/shared'; // Disabled for iOS
import { handleApiError } from './errorHandler';

export class RealAuthService implements AuthService {
  async login(email: string, password: string) {
    try {
      // Fallback implementation for iOS - should not be used since we're in mock mode
      throw new Error('Real service not available on iOS - use mock mode');
